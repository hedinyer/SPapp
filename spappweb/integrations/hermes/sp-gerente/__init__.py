"""Plugin Hermes — reportes de puntos de venta SP (solo lectura vía Supabase).

Sedés:
  garrido  → Soluciones Garrido (Girardot) — proyecto iilgrapnrkwdcouielwz
  pinilla  → Soluciones Pinilla (Bogotá)   — proyecto ziihqvtjacqzwmcmpiyp

Env opcionales (override de defaults embebidos):
  SP_GARRIDO_SUPABASE_URL / SP_GARRIDO_SUPABASE_ANON_KEY
  SP_PINILLA_SUPABASE_URL / SP_PINILLA_SUPABASE_ANON_KEY
"""
...
# Defaults = anon keys ya públicas en cada spappweb/src/lib/supabase/public-env.ts
_SEDES: dict[str, dict[str, str]] = {
    "garrido": {
        "label": "Soluciones Garrido S.A.S. (Girardot)",
        "alias": "garrido",
        "url": os.environ.get(
            "SP_GARRIDO_SUPABASE_URL",
            "https://iilgrapnrkwdcouielwz.supabase.co",
        ).rstrip("/"),
        "key": os.environ.get(
            "SP_GARRIDO_SUPABASE_ANON_KEY",
    ...
    "pinilla": {
        "label": "Soluciones Pinilla S.A.S. (Bogotá)",
        "alias": "pinilla",
        "url": os.environ.get(
            "SP_PINILLA_SUPABASE_URL",
            "https://ziihqvtjacqzwmcmpiyp.supabase.co",
        ).rstrip("/"),
        "key": os.environ.get(
            "SP_PINILLA_SUPABASE_ANON_KEY",
        ...
        "panel": "https://sp-bogota.vercel.app",
    },
}
...
def _resolve_sedes(sede: str | None) -> list[str]:
    s = (sede or "ambas").strip().lower()
    if s in ("ambas", "all", "*"):
        return ["garrido", "pinilla"]
    if s in _SEDES:
        return [s]
    raise ValueError("sede debe ser garrido | pinilla | ambas")
...
def _for_each_sede(sede: str | None, fn):
    ...
    return {"ok": True, "modo": "ambas", "sedes": out}
    ...
    return _ok(
        {
            ...
            "nota": "Usa sede=garrido|pinilla|ambas en cada tool. Solo lectura.",
        }
    )
...
def handle_cliente(params, **kwargs):
    ...
    try:
        ...
        if sede not in _SEDES:
            return _err("sp_cliente requiere sede=garrido|pinilla (no ambas)")
...
_SEDE_PROP = {
    "type": "string",
    "enum": ["garrido", "pinilla", "ambas"],
    "description": "Punto de venta. Default: ambas.",
}
_FECHA_PROP = {
    "type": "string",
...
_TOOLS = [
    (
        "sp_sedes",
        "Lista los puntos de venta SP (Garrido y Pinilla) y sus paneles.",
    ...
    (
        "sp_cliente",
        "Ficha 360° de un cliente (user_id) en UNA sede: docs, contrato, compra, atraso, pagos.",
        {
            "type": "object",
            "properties": {
                "sede": {
                    "type": "string",
                    "enum": ["garrido", "pinilla"],
                    "description": "Obligatorio (no ambas).",
                },
                "user_id": {"type": "integer"},
            },
            "required": ["sede", "user_id"],
        },
        handle_cliente,
    ),
    (
...
def register(ctx):
    ...
    print(f"[sp-gerente] {len(_TOOLS)} tools de reportes PDV (Garrido+Pinilla, solo lectura).")


# ponytail: self-check mínimo; falla si Supabase no responde en al menos una sede
def _selfcheck() -> None:
    data = json.loads(handle_sedes({}))
    assert data.get("ok") and len(data.get("sedes", [])) == 2
    inv = json.loads(handle_inventario({"sede": "garrido", "solo_alertas": True}))
    assert inv.get("ok") is not False or "error" in inv
    print("[sp-gerente] selfcheck ok:", {"sedes": 2, "garrido_inv_ok": inv.get("ok", True)})
