/// Texto legal verbatim del Contrato de Renting (PLANTILLA_CONTRATO_COMPRAVENTA.pdf).
/// Los placeholders comerciales se marcan con [COMERCIAL].
class ContratoRentingClausulas {
  static const introTemplate = '''
El día [DIA] del mes de [MES] de [ANIO], en la ciudad de Bucaramanga, Santander, entre los suscrito a saber,
...
cláusulas:''';
  ...
  static const firmaTemplate = '''
Para constancia se firma en Bucaramanga a los [DIA] (xx) día del mes de [MES] de dos mil [ANIO] ([ANIO_NUM]), por quienes en el intervinieron.
...
Nit: 901.397.015-2''';
  ...
  static String renderIntro(ContratoRentingFormData form) {
  ...
  static String renderClausulaTexto(String texto, ContratoRentingFormData form) {
    return texto.replaceAll(
      '[DIRECCION_NOTIFICACIONES]',
      form.direccionNotificaciones.isNotEmpty
          ? form.direccionNotificaciones
...
class ClausulaBlock {
  const ClausulaBlock({required this.title, required this.clausulas});
...
class Clausula {
  const Clausula({required this.titulo, required this.texto});

  final String titulo;
  final String texto;
}

class ContratoRentingFormData {
  const ContratoRentingFormData({
